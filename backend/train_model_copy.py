...
class NIHChestXrayDataset(Dataset):
    def __init__(self, hf_dataset, transform=None, split='train', max_samples=None):
        ...
        print("Label type:", type(first_item['label']))
        print("Label value:", first_item['label'])
    ...
    def __getitem__(self, idx):
        try:
            ...
            if isinstance(item['label'], str):
                conditions = item['label'].split('|')
            elif isinstance(item['label'], (list, np.ndarray)):
                conditions = item['label']
            else:
                print(f"⚠️  Unexpected label type: {type(item['label'])}")
                conditions = []

            for condition in conditions:
                if condition in NIH_LABEL_MAPPING:
                    mapped_label = NIH_LABEL_MAPPING[condition]
                    if mapped_label in LABELS:
                        label_idx = LABELS.index(mapped_label)
                        labels[label_idx] = 1.0

            if idx < 5:
                print(f"Converted labels for sample {idx}: {labels}")

            return image, labels
        except Exception as e:
            print(f"⚠️  Error loading sample {idx}: {str(e)}")
...
def train_model(args):
    ...
    try:
        ...
        print("Label example:", first_sample['label'])
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        while processed_samples < samples_per_epoch:
            try:
                ...
                labels = torch.zeros(len(LABELS), dtype=torch.float32)
                label_assignments = []
                for nih_label, our_label in NIH_LABEL_MAPPING.items():
