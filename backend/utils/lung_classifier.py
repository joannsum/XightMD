...
class LungClassifierTrainer:
    def __init__(self, model_path: str = None):
        ...
        # IMPORTANT: Use SimpleLungModel for balanced_lung_model.pth
        # The balanced model was trained with ResNet18, not EfficientNet
        self.model = self._create_model_for_checkpoint(model_path)
    ...
    def _create_model_for_checkpoint(self, model_path: str):
        """Create appropriate model based on checkpoint"""
        if model_path and 'balanced' in model_path:
            # Use SimpleLungModel for balanced checkpoints
            import torchvision.models as models
            ...
            return SimpleLungModel()
        else:
            # Use OptimizedLungClassifier for other models
            return OptimizedLungClassifier(num_classes=len(LABELS))
    ...
    def load_model(self, path: str):
        ...
        try:
            if 'balanced' in path:
                # Load state dict directly for balanced model
                state_dict = torch.load(path, map_location=self.device)
                self.model.load_state_dict(state_dict)
                print(f"✅ Balanced model loaded from {path}")
            else:
                # Load full checkpoint for other models
